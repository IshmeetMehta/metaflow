...
- Google Cloud Storage ("Storage Object Admin" on bucket ${var.storage_bucket_name})
...
Ask for the pregenerated service account key (${var.service_account_key_file}) from the administrator (the person who stood up the Metaflow stack).
...
$ gcloud container clusters get-credentials ${var.kubernetes_cluster_name} --region=${var.zone}
