...
from metaflow import (
    ...
    timeout,
    retry,
    parallel, # <-- 1. Imported the official parallel primitive
)

IMAGE_ARGS = {
    # v2 image asset
    #v2 contains dependencies mentioned in the Dockerfile
    "base_image": "us-west1-docker.pkg.dev/platform-sandbox-495413/spsearch-qualitycheck-metaflow-image/sp-search-firelight-qualitycheck:v2",
}
ENV_ARGS = {
    "vars": {
...
class MtmlV35V2c(FlowSpec):
    ...
    def start_run_preproc_train(self):
        self.next(self.run_preproc_train, num_parallel=1)

    @environment(**ENV_ARGS)
    @timeout(hours=48)
    @kubernetes(
        cpu=20,          # CORRECTED: Fits within the 24 vCPU host limit
        gpu=2,           # CORRECTED: Matches the 2x L4 GPUs on g2-standard-24
        memory=80000,    # CORRECTED: Fits within the 96 GB host RAM limit (80 GB)
        namespace="metaflow",
        shared_memory=10000,
    ...
    @retry(times=2)
    @parallel # <-- 2. Placed here to fulfill num_parallel scheduling conditions
    @step
    def run_preproc_train(self):
        ...
        code_directory = os.environ.get("METAFLOW_CODE_DIR", "/workspace")
        target_script = os.path.join(code_directory, "run_preproc.py")
    ...
    def start_train(self):
        self.next(self.train, num_parallel=1)
    ...
    @retry(times=2)
    @parallel # <-- 3. Also placed here since start_train splits into num_parallel=2
    @step
    def train(self):
        import os
        import subprocess
        ...
        code_directory = os.environ.get("METAFLOW_CODE_DIR", "/workspace")
        target_train_script = os.path.join(code_directory, "train.py")
